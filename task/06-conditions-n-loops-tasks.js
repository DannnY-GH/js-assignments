...
function getSumBetweenNumbers(n1, n2) {
    return Math.floor((n1 + n2)*(n2 - n1 + 1) / 2);
}
...
function isCreditCardNumber(ccn) {
    ...
    ccn = ccn.reduce((acc, val, ind)=>{
        var dig = Number(val);
        if(ind % 2)
            dig *= 2;
        acc += Math.floor(dig / 10);
        acc += dig % 10;
        return acc;
    }, 0);
    return (ccn * 3) % 10 == 0;
...
function getMatrixProduct(m1, m2) {
    var ans = [...Array(m1.length)].map(e=>Array(m2[0].length).fill(0));
...
function evaluateTicTacToePosition(position) {
    ...
    function cv(in_arr) {
        ...
        for (var i = 0; i < 3; i++)
            for (var j = 0; j < 3; j++)
                arr[i][j] = in_arr[_map[i][j][0]][_map[i][j][1]];
        return arr;
    }
    var patterns = [
        [[0, 0], [0, 1], [0, 2]],
    ...
    function check() {
        ...
        for (var i = 0; i < 4; i++) {
            for (var j = 0; j < patterns.length; j++) {
                var ptr = patterns[j];
                if (arr[ptr[0][0]][ptr[0][1]] ==
                    arr[ptr[1][0]][ptr[1][1]] &&
                    arr[ptr[1][0]][ptr[1][1]] ==
                    arr[ptr[2][0]][ptr[2][1]] &&
                    arr[ptr[0][0]][ptr[0][1]] != undefined){
                        var tmp = arr[ptr[0][0]][ptr[0][1]];
                        if(tmp == 'X')
                            X |= true;
                        else
                            O |= true;
